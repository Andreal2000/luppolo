...
class LuppoloAstVisitor(LuppoloVisitor):
    """
    LuppoloAstVisitor is responsible for traversing and transforming the parse tree produced by the LuppoloParser into an Abstract Syntax Tree (AST).
    It extends the base visitor generated by ANTLR (LuppoloVisitor) and overrides the necessary methods to visit and process different language constructs, converting them into their respective AST nodes.

    Each method in this class corresponds to a specific rule in the LuppoloParser grammar and is responsible for handling
    the parse tree node produced by that rule.
    """
